from sqlalchemy import Column, Integer, String, Float, ForeignKey, DateTime, Text, Boolean, UniqueConstraint, JSON
...
class Product(Base):
    ...
    # Will store JSON list or pipe-separated string
    advices = Column(String, nullable=True)
    ...
    themes = relationship("Theme", back_populates="product",
...
class Review(Base):
    ...
    claims = relationship("Claim", back_populates="review",
                          cascade="all, delete-orphan")
...
class Claim(Base):
    ...
    theme_id = Column(Integer, ForeignKey(
        "themes.id"), nullable=True, index=True)
...
class AmazonProduct(Base):
    ...
    native_reviews = relationship(
        "NativeReview", back_populates="amazon_product", cascade="all, delete-orphan")
    amazon_reviews = relationship(
        "AmazonReview", back_populates="amazon_product", cascade="all, delete-orphan")
...
class AmazonReview(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    amazon_product_asin = Column(String, ForeignKey(
        "amazon_products.asin"), index=True, nullable=False)
    canopy_id = Column(String, unique=True, index=True, nullable=False)
    ...
    # Relationships
    amazon_product = relationship(
        "AmazonProduct", back_populates="amazon_reviews")
...
class NativeReview(Base):
    ...
    # Display name (not auth-linked for now)
    ...
    __table_args__ = (
        UniqueConstraint('amazon_product_asin', 'device_id',
                         name='uq_native_review_device'),
    )
